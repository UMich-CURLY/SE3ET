...
class ThreeDMatchPairSmallDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        dataset_root,
        subset,
        ...
        super(ThreeDMatchPairSmallDataset, self).__init__()
        ...
        with open(osp.join(self.metadata_root, f'{subset}.pkl'), 'rb') as f:
            ...
            if subset == 'train':
                self.metadata_list =  self.metadata_list[:2000]
            elif subset == 'val':
                self.metadata_list =  self.metadata_list[:130]
