...
class OdometryKittiPairTestDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        super(OdometryKittiPairTestDataset, self).__init__()
    ...
    def _load_point_cloud(self, file_name):
        ...
        if self.point_limit is not None and points.shape[0] > self.point_limit:
            ...
            close_by_points = points[np.linalg.norm(points, axis=1) < 30]
            indices = np.random.permutation(close_by_points.shape[0])[: self.point_limit]
            points = close_by_points[indices]
            points = 0.1 * points
    ...
    def __getitem__(self, index):
        ...
        transform[:3, 3] = transform[:3, 3] * 0.1  # scaling to match 3DMatch
