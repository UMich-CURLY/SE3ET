...
from backbone import E2PN, E2PN_normal
...
class GeoTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.backbone_normal = E2PN_normal(
            cfg.backbone.input_dim,
            cfg.backbone.output_dim,
            cfg.backbone.init_dim,
            cfg.backbone.init_radius,
    ...
    def forward(self, data_dict):
        ...
        normals_c = data_dict['normals'][-1].detach()
        ...
        feats_c = feats_list[-1] # N, A, C=512
        feats_f = feats_list[0] # N, C=128

        # 2.1 Normal Encoder
        feats_normal_list = self.backbone_normal(feats, data_dict)
        feats_normal_c = feats_normal_list[-1]
        feats_normal_f = feats_normal_list[0]

        # combine
        feats_c = torch.cat((feats_c, feats_normal_c), 2)
        feats_f = torch.cat((feats_f, feats_normal_f), 1)
