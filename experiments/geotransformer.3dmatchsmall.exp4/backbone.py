...
class KPConvFPN(nn.Module):
    def __init__(self, input_dim, output_dim, init_dim, kernel_size, init_radius, init_sigma, group_norm):
        ...
        self.encoder1_2 = ResidualBlock(init_dim, init_dim * 2, kernel_size, init_radius, init_sigma, group_norm, strided=True)

        self.encoder2_1 = ResidualBlock(
            init_dim * 2, init_dim * 4, kernel_size, init_radius * 2, init_sigma * 2, group_norm, strided=True
        )

        self.encoder3_1 = ResidualBlock(
            init_dim * 4, init_dim * 8, kernel_size, init_radius * 4, init_sigma * 4, group_norm, strided=True
        )

        self.decoder3 = UnaryBlock(init_dim * 12, init_dim * 4, group_norm)
        self.decoder2 = LastUnaryBlock(init_dim * 6, output_dim)
    ...
    def forward(self, feats, data_dict):
        ...
        latent_s2 = self.decoder3(latent_s2)
        ...
        latent_s1 = self.decoder2(latent_s1)
