...
class GeoTransformer(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # save features
        torch.save(ref_feats_c, 'encoder_ref_feats_c.pt')
        torch.save(src_feats_c, 'encoder_src_feats_c.pt')

        # test similarity
        cossim = nn.CosineSimilarity(dim=1, eps=1e-6)
        cossim_enc_01 = torch.mean(cossim(ref_feats_c[:, 0, :], ref_feats_c[:, 1, :]))
        cossim_enc_12 = torch.mean(cossim(ref_feats_c[:, 1, :], ref_feats_c[:, 2, :]))
        cossim_enc_23 = torch.mean(cossim(ref_feats_c[:, 2, :], ref_feats_c[:, 3, :]))

        print('cossim_enc_01', cossim_enc_01)
        print('cossim_enc_12', cossim_enc_12)
        print('cossim_enc_23', cossim_enc_23)
        ...
        # save features
        torch.save(ref_feats_m, 'transformer_ref_feats_c.pt')
        torch.save(src_feats_m, 'transformer_src_feats_c.pt')

        # test similarity
        print('ref_feats_m', ref_feats_m.shape)
        print('ref_feats_m[:, 0, :]', ref_feats_m[:, 0, :].shape)
        cossim_trans_01 = cossim(ref_feats_m[:, 0, :], ref_feats_m[:, 1, :])
        cossim_trans_12 = cossim(ref_feats_m[:, 1, :], ref_feats_m[:, 2, :])
        cossim_trans_23 = cossim(ref_feats_m[:, 2, :], ref_feats_m[:, 3, :])
        cossim_trans_34 = cossim(ref_feats_m[:, 3, :], ref_feats_m[:, 4, :])
        cossim_trans_45 = cossim(ref_feats_m[:, 4, :], ref_feats_m[:, 5, :])

        print('cossim_trans_01', cossim_trans_01)
        print('cossim_trans_12', cossim_trans_12)
        print('cossim_trans_23', cossim_trans_23)
        print('cossim_trans_34', cossim_trans_23)
        print('cossim_trans_45', cossim_trans_23)
