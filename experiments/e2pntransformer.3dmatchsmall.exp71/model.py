...
class GeoTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.rotation_supervision = RotationAttentionLayer(cfg.geotransformer.hidden_dim, cfg.geotransformer.num_heads)
    ...
    def forward(self, data_dict):
        ...
        # print('differences between ref_points and src_points:', torch.norm(ref_points - src_points))
        # print('differences between ref_points_f and src_points_f:', torch.norm(ref_points_f - src_points_f))
        # print('differences between ref_points_c and src_points_c:', torch.norm(ref_points_c - src_points_c))
        ...
        if self.training:
            ref_feats_c, src_feats_c, ref_feats_m, src_feats_m, attn_matrix0, attn_matrix1 = self.transformer(
                ...
                gt_indices=gt_node_corr_indices,
                gt_overlap=gt_node_corr_overlaps,
                # ref_node_masks,
                # src_node_masks,
            ) # B, N/M, C=256
        else:
        ...
        ref_feats_c_norm = F.normalize(ref_feats_c.squeeze(0), p=2, dim=1) # N, C=256
        src_feats_c_norm = F.normalize(src_feats_c.squeeze(0), p=2, dim=1) # M, C=256

        output_dict['ref_feats_c'] = ref_feats_c_norm
        output_dict['src_feats_c'] = src_feats_c_norm

        # point matching supervision in the middle of the transformer network
