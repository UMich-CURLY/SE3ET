...
class MidPointMatchingLoss(nn.Module):
    def __init__(self, cfg):
        super(MidPointMatchingLoss, self).__init__()
        self.weighted_circle_loss = WeightedCircleLoss(
            cfg.coarse_loss.positive_margin,
            cfg.coarse_loss.negative_margin,
            cfg.coarse_loss.positive_optimal,
    ...
    def forward(self, output_dict):
        ref_feats = output_dict['ref_feats_m']
        src_feats = output_dict['src_feats_m']
        gt_node_corr_indices = output_dict['gt_node_corr_indices']
        gt_node_corr_overlaps = output_dict['gt_node_corr_overlaps']
        gt_ref_node_corr_indices = gt_node_corr_indices[:, 0]
        gt_src_node_corr_indices = gt_node_corr_indices[:, 1]
...
class RotationMatchingLoss(nn.Module):
    def __init__(self, cfg):
        ...
        if self.na == 4:
            ...
            vs, v_adjs, v_level2s, v_opps, vRs = sptk.get_icosahedron_vertices()
            self.adj0 = v_adjs[0,0]
            self.anchors = nn.Parameter(torch.tensor(vRs, dtype=torch.float32), requires_grad=False)  # 12*3*3
            self.trace_idx_ori, _ = L.get_relativeV_index()
        else:
            raise NotImplementedError(f"kanchor={self.na} is not implemented in the RotationMatchingLoss()")
        
        self.attn_r_positive_rot_supervise = cfg.geotransformer.attn_r_positive_rot_supervise
...
class OverallLoss(nn.Module):
    def __init__(self, cfg):
        ...
        self.mid_loss = MidPointMatchingLoss(cfg)
        ...
        self.weight_mid_loss = cfg.loss.weight_mid_loss
    ...
    def forward(self, output_dict, data_dict):
        ...
        if self.supervise_rotation:
            ...
            # mid_loss = self.mid_loss(output_dict)
            # loss = self.weight_coarse_loss * coarse_loss + self.weight_fine_loss * fine_loss + self.weight_rotation_loss * rotation_loss + self.weight_mid_loss * mid_loss
