...
class GeoTransformer(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # # # save points from GeoTransformer preprocessing 
        # ref_length_c = data_dict['lengths'][-1][0].item()
        # ref_length_f = data_dict['lengths'][1][0].item()
        # points = data_dict['points'][0].detach() # raw point cloud
        # points_f = data_dict['points'][1].detach() # fine point cloud
        # points_c = data_dict['points'][3].detach() # course point cloud
        # ref_points_save = points[:ref_length].clone().detach().cpu()
        # src_points_save = points[ref_length:].clone().detach().cpu()
        # ref_points_c_save = points_c[:ref_length_c].clone().detach().cpu()
        # src_points_c_save = points_c[ref_length_c:].clone().detach().cpu()
        # ref_points_f_save = points_f[:ref_length_f].clone().detach().cpu()
        # src_points_f_save = points_f[ref_length_f:].clone().detach().cpu()
        # import numpy as np
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_ref_points.npy', ref_points_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_src_points.npy', src_points_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_geo_ref_points_c.npy', ref_points_c_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_geo_src_points_c.npy', src_points_c_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_geo_ref_points_f.npy', ref_points_f_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_geo_src_points_f.npy', src_points_f_save)
        ...
        # # # save point clouds from E2PN backbone
        # ref_points_c_save = ref_points_c.clone().detach().cpu()
        # src_points_c_save = src_points_c.clone().detach().cpu()
        # ref_points_f_save = ref_points_f.clone().detach().cpu()
        # src_points_f_save = src_points_f.clone().detach().cpu()
        # import numpy as np
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_e2pn_ref_points_c.npy', ref_points_c_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_e2pn_src_points_c.npy', src_points_c_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_e2pn_ref_points_f.npy', ref_points_f_save)
        # np.save('/home/cel/data/3DMatch/debug_vis/4096_e2pn_src_points_f.npy', src_points_f_save)
        ...
        ref_feats_c, src_feats_c, _, _ = self.transformer(
