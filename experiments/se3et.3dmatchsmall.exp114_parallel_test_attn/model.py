...
class SE3ET(nn.Module):
    def __init__(self, cfg):
        super(SE3ET, self).__init__()
        ...
        self.transformer2 = GeometricTransformer(
            cfg.geotransformer2.input_dim,
            cfg.geotransformer2.output_dim,
            cfg.geotransformer2.hidden_dim,
            cfg.geotransformer2.num_heads,
            cfg.geotransformer2.blocks,
            cfg.geotransformer2.sigma_d,
            cfg.geotransformer2.sigma_a,
            cfg.geotransformer2.angle_k,
            supervise_rotation=cfg.geotransformer2.supervise_rotation,
            reduction_a=cfg.geotransformer2.reduction_a,
            na=cfg.epn.kanchor,
            align_mode=cfg.geotransformer2.align_mode,
            alternative_impl=cfg.geotransformer2.alternative_impl,
            n_level_equiv=cfg.geotransformer2.n_level_equiv,
        )       
    ...
    def forward(self, data_dict):
        ...
        ref_feats_c_inv = torch.amax(ref_feats_c, 1, keepdim=False) # nac -> nc
        src_feats_c_inv = torch.amax(src_feats_c, 1, keepdim=False) # nac -> nc

        # invariant geotransformer for point matching
        ref_feats_c_inv, src_feats_c_inv, _, _, _, _ = self.transformer(
            ref_points_c.unsqueeze(0),
            src_points_c.unsqueeze(0),
            ref_feats_c_inv.unsqueeze(0),
            src_feats_c_inv.unsqueeze(0),
        ) # B, N/M, C=256

        # equivariant geotransformer for rotation supervision
        ref_feats_c_equi, src_feats_c_equi, _, _, attn_matrix0, attn_matrix1 = self.transformer2(
            ref_points_c.unsqueeze(0),
            src_points_c.unsqueeze(0),
            ref_feats_c_equi.unsqueeze(0),
            src_feats_c_equi.unsqueeze(0),
        ) # B, N/M, C=256

        output_dict['attn_matrix0'] = attn_matrix0
        output_dict['attn_matrix1'] = attn_matrix1

        ref_feats_c_inv_norm = F.normalize(ref_feats_c_inv.squeeze(0), p=2, dim=1)
        src_feats_c_inv_norm = F.normalize(src_feats_c_inv.squeeze(0), p=2, dim=1)
        ref_feats_c_equi_norm = F.normalize(ref_feats_c_equi.squeeze(0), p=2, dim=1)
        src_feats_c_equi_norm = F.normalize(src_feats_c_equi.squeeze(0), p=2, dim=1)

        output_dict['ref_feats_c'] = ref_feats_c_inv_norm
        output_dict['src_feats_c'] = src_feats_c_inv_norm
        ...
        with torch.no_grad():
            ref_node_corr_indices, src_node_corr_indices, node_corr_scores = self.coarse_matching(
                ref_feats_c_inv_norm, src_feats_c_inv_norm, ref_node_masks, src_node_masks
            )
