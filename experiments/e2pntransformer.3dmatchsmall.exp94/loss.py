...
class CoarseAnchorMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict):
        ...
        spatial_loss = self.weighted_circle_loss(pos_masks, neg_masks, feat_dists, pos_scales)

        # anchor matching. (1) use gt corr to find matching points
        ref_feats_matching = ref_feats[:, gt_ref_node_corr_indices, :] # anc -> an'c
        src_feats_matching = src_feats[:, gt_src_node_corr_indices, :] # amc -> an'c

        # anchor matching. (2) compute distance between anchors
        ref_feats_permute = torch.permute(ref_feats_matching, (1,0,2)) # n'ac
        src_feats_permute = torch.permute(src_feats_matching, (1,0,2)) # n'ac
        anchor_feat_dists = torch.sqrt(pairwise_distance(ref_feats_permute, src_feats_permute, normalized=True)) # n'aa

        # anchor matching. (3) create postivie and negative masks for mathing
        anchor_matching = torch.eye(self.na).unsqueeze(0).to(anchor_feat_dists.device)
        anchor_matching = anchor_matching.expand(anchor_feat_dists.shape[0], self.na, self.na)
        anchor_pos_masks = torch.eq(anchor_matching, 1) # n'aa
        anchor_neg_masks = torch.eq(anchor_matching, 0) # n'aa
        anchor_pos_scales = torch.sqrt(anchor_matching * anchor_pos_masks.float())

        # print('anchor_feat_dists', anchor_feat_dists.shape)
        # print('anchor_pos_masks', anchor_pos_masks)
        # print('anchor_neg_masks', anchor_neg_masks)
        # print('anchor_pos_scales', anchor_pos_scales)


        # anchor matching. (4) compute loss
        anchor_loss = self.weighted_circle_loss(anchor_pos_masks, anchor_neg_masks, anchor_feat_dists, anchor_pos_scales)

        return spatial_loss, anchor_loss
...
class OverallLoss(nn.Module):
    def __init__(self, cfg):
        ...
        self.weight_spatial_loss = cfg.loss.weight_coarse_loss
        self.weight_fine_loss = cfg.loss.weight_fine_loss
        self.weight_anchor_loss = cfg.loss.weight_anchor_loss
    ...
    def forward(self, output_dict, data_dict):
        if self.anchor_matching:
            ...
            saptial_loss, anchor_loss = self.coarse_anchor_loss(output_dict)
        ...
        if self.supervise_rotation:
            ...
            loss = self.weight_spatial_loss * saptial_loss + self.weight_fine_loss * fine_loss
